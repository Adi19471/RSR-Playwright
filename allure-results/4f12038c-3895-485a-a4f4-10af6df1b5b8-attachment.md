...
Expected substring: "AT1783074113721"
...
Call log:
  ...
  - waiting for getByRole('grid')
    22 × locator resolved to <div role="grid" tabindex="-1" aria-colcount="5" aria-rowcount="1" aria-multiselectable="false" class="MuiDataGrid-main css-1rcz4kn">…</div>
...
```ts
  31  |   await createAdminRecord(page, [
  32  |     { label: "Code", value: code },
  33  |     { label: "Description", value: `AUTOTEST-${code}` },
  34  |   ]);
  35  |   await searchAdminGrid(page, code);
  ...
  121 |   const code = `AT${Date.now()}`;
  122 |   await openAdminScreens(page);
  123 |   await openMasterScreen(page, "Machinery");
  124 |   await openNewRecordForm(page);
  125 |   await createAdminRecord(page, [
  126 |     { label: "Code", value: code },
  127 |     { label: "Group", kind: "combobox" },
  128 |     { label: "Description", value: `AUTOTEST-${code}` },
  129 |   ]);
  130 |   await searchAdminGrid(page, code);
> 131 |   await expect(page.getByRole("grid")).toContainText(code);
      |                                        ^ Error: expect(locator).toContainText(expected) failed
  132 | });
  133 | 
```